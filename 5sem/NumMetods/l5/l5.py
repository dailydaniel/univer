...
def createParser():
    ...
    parser.add_argument('-d', '--data', default='l5.txt', type=str)
    return parser
...
def makeArr(a: list, X: float, x=None, y=None) -> float:
    '''возвращение значения многочлена в точке X'''
    return reduce(lambda a, b: a + b[0] * X ** b[1], preduce(list(zip(a, range(len(a))))))

def qd(x: list, y: list, x_test: list, y_test: list, A: list, fn=makeArr, delta=5) -> float:
    '''квадратичное отклонение'''
    return round(reduce(lambda a, b: a + (fn(A, b[0], x, y) - b[1]) ** 2, preduce(list(zip(x_test, y_test)))), delta)

def lagr(A: list, X: float, x: list, y: list, delta=5) -> float:
    '''многочлен лагранжа'''
    res = 0
